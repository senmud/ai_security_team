...
- **公网搜索**：扩展工具 **web_search**（`skills.py`），默认与已安装 Skills 一并作为 **`primary_security_tools()`** 挂载。
- **占位安全 + ClawHub**：**`default_security_tools()`**（不含本地 skill 与 `web_search`）；仅在首轮 `stream` **异常**时由 **`stream_security_agent_with_fallback`** 换用，与首轮互补。
...
**工具绑定（重要）**：你只能调用**当前实际绑定**给你的工具。首轮运行通常仅挂载「已安装扩展 Skills」与 `web_search`；若列表中**没有** `threat_feed_connector`、`clawhub_search_skills` 等，则**不要**尝试调用它们。若首轮执行**异常失败**，系统会自动换用含占位安全工具与 ClawHub 的扩展集**重试一轮**。
...
def primary_security_tools() -> List[BaseTool]:
    """首轮 Deep Agent：仅合并本地已安装 Skills + `web_search`（不含占位安全工具与 ClawHub）。"""
    return [*_installed_skill_tools(), web_search]


def default_security_tools() -> List[BaseTool]:
    """
    回退轮次使用：占位安全工具 + ClawHub（**不含**本地已安装 skill 与 `web_search`）。
    与 `primary_security_tools` 互补，在首轮流式执行异常时由 `stream_security_agent_with_fallback` 挂载。
    """
    return [
...
def stream_security_agent_with_fallback(
    model: BaseChatModel,
    ...
    *,
    ...
    """
    先以 `primary_security_tools()` 构建 Agent 并 `stream`；若在消费流过程中抛出异常，再以
    ...
    for use_fallback in (False, True):
        try:
            ...
            agent = create_security_deep_agent(
                model,
                tools=tools,
                use_local_workspace_and_shell=use_local_workspace_and_shell,
                backend=backend,
            )
...
def create_security_deep_agent(
    ...
    *,
    ...
    - `tools=None`（默认）：仅挂载 **`primary_security_tools()`**（本地已安装 Skills + `web_search`）。
    - 传入非空 `tools`：按调用方指定合并（例如回退时使用 `default_security_tools()`）。
    ...
    需要「首轮 skill+搜索、失败再换完整扩展集」时，请使用 **`stream_security_agent_with_fallback`**，而不是在本函数外手写两套逻辑。
    """
    ...
    resolved_tools = primary_security_tools() if tools is None else tools
